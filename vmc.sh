...
CENTOS_8_URL="https://cloud.centos.org/centos/8-stream/x86_64/images/CentOS-Stream-GenericCloud-8-20210603.0.x86_64.qcow2"
...
virt-customize -a $TEMPLATE_VM_CI_IMAGE --install qemu-guest-agent,curl,wget --run-command "curl -fsSL get.docker.com -o get-docker.sh && sh get-docker.sh" --install docker-compose
